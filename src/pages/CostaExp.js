				...
				<>
					...
					<div className="two-column-containercs mx-3">
						<div className="left-columncs">
							<h2 className="l1cs PoppExBold">A new way to travel</h2>
						</div>
						<div className="right-columncs">
							<p className="l2cs PoppMed">
							...
							<div key={image.id} className={`row rowcs ${(index + 1) % 2 === 0 ? "flex-row-reverse" : ""}`}>
								...
								<div className="col-lg-6 d-flex">
									<div className={`text-container align-self-end ${(index + 1) % 2 !== 0 ? "right-textcs" : "left-textcs"}`}>
										<h4 className={`PoppMed ${(index + 1) % 2 !== 0 ? "category-style-rightcs" : "category-style-leftcs"}`}>{image.category}</h4>
										<h3 className={`PoppExBold ${(index + 1) % 2 !== 0 ? "title-style-rightcs" : "title-style-leftcs"}`}><span dangerouslySetInnerHTML={{ __html: image.title }} /></h3>
										<p className={`PoppMed ${(index + 1) % 2 !== 0 ? "text-style-rightcs" : "text-style-leftcs"}`}>
											{!readMore[index] ? (
												<>
													<span>{image.text1}{' '}
														<a href="#" className="clickable-text" onClick={() => xyz(index)}>
															See More
														</a>
													</span>
												</>
